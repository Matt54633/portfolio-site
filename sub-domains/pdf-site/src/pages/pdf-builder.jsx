        ...
        const handleResize = () => {
            if (documentRef.current) {
                ...
                const scaleFactor = Math.max(
                    0.4,
                    (viewportWidth / minWidth) * 0.98,
                )
                const scale = Math.min(
                    1,
                    scaleFactor * (viewportWidth / window.innerWidth),
                )
            ...
            >
                <div className="flex w-full flex-col gap-4 lg:w-1/4">
                    ...
                    <button
                        ...
                        disabled={isLoading}
                        className={`w-full rounded-lg bg-scout-purple px-2 py-2.5 font-[Nunito] font-bold text-white transition-all duration-200 hover:bg-scout-purple-hover disabled:bg-black`}
                    >
                        {isLoading ? "Generating PDF" : "Download PDF"}
                    </button>
                </div>
                <div className="flex w-full flex-col gap-4 overflow-x-auto rounded-lg bg-gray-100 py-2 lg:w-3/4">
                    <h1 className="border-b-[3px] px-4 pb-1.5 font-[Nunito] text-lg font-bold">
                        Document Preview
                    </h1>

                    <div className="w-full overflow-x-auto px-4 pb-2.5">
                        <div
                            className="min-w-[750px] lg:w-full lg:min-w-0"
                            ref={documentRef}
                        >
                            <div className="rounded-lg border-[3px] border-gray-200">
